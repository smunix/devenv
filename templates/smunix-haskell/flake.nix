        ...
        slow pkgs.haskell.packages."ghc${ghc-version}" [
          {
            ...
            extension = hfinal: hprevious:
              with hfinal; {
                project = callCabal2nixWithOptions "project"
                  (filter {
        ...
        in
        {
          default = devenv.lib.mkShell rec {
            ...
            modules = [{
              ...
              '';
              scripts = {
                repl-lib.exec = ''ghcid -W -a -T "Lib.Top.main" -c cabal repl lib:project'';
                repl-app.exec = ''ghcid -W -a -T "Top.main" -c cabal repl exe:project-exe'';
              };
              pre-commit.hooks = {
                nixfmt.enable = true;
